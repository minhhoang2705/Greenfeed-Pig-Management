...
if __name__ == '__main__':
    # Example usage
    detector = PigDetector(model_path='yolov5s') # Using pretrained yolov5s
    
    # Load an example image
    try:
        image = cv2.imread('test_image.jpg') # Replace with your image path
        if image is None:
            raise FileNotFoundError("Could not read image file. Please make sure 'test_image.jpg' exists in the same directory.")
